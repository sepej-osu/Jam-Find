...
def test_like_count_multiple_users():
    """Likes from two different users both count; each sees their own likedByCurrentUser."""
    # Create a fresh post
    resp = client.post(
        "/api/v1/posts",
        json={
            "title": "Multi-user like test",
            "body": "Testing that multiple distinct users can like the same post",
            "postType": "looking_to_jam",
        },
    )
    ...
    try:
        # User 1 (DEV_USER_ID) likes the post
        resp = client.post(f"/api/v1/posts/{post_id}/like")
        assert resp.status_code == 200
        assert resp.json()["likes"] == 1
        assert resp.json()["liked"] is True

        # Switch to user 2
        app.dependency_overrides[get_current_user] = lambda: settings.DEV_USER_ID_2

        # User 2 likes the same post — count should now be 2
        resp = client.post(f"/api/v1/posts/{post_id}/like")
        assert resp.status_code == 200
        assert resp.json()["likes"] == 2
        assert resp.json()["liked"] is True

        # User 2 fetches the post — likedByCurrentUser should be True
        resp = client.get(f"/api/v1/posts/{post_id}")
        assert resp.status_code == 200
        assert resp.json()["likes"] == 2
        assert resp.json()["likedByCurrentUser"] is True

        # Switch back to user 1 — still liked, count still 2
        app.dependency_overrides[get_current_user] = override_get_current_user
        resp = client.get(f"/api/v1/posts/{post_id}")
        assert resp.status_code == 200
        assert resp.json()["likes"] == 2
        ...
        # User 1 unlikes — count drops to 1, user 2's like remains
        resp = client.post(f"/api/v1/posts/{post_id}/like")
        assert resp.status_code == 200
        assert resp.json()["likes"] == 1
